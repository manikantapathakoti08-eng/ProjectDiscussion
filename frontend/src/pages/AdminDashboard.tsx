...
import { getAllUsers, onboardUser } from '../api/admin.api';
import { LogOut, Users, Check, Loader2, Filter, UserPlus, Phone, Mail as MailIcon, Hash, BadgeCheck, XCircle } from 'lucide-react';
                     ...
                     <form key={onboardingRole} onSubmit={(e) => { e.preventDefault(); onboardMutation.mutate({ ...onboardForm, role: onboardingRole }); }} className="flex-col gap-5 animate-fade-up">
                        ...
                        {onboardingRole === 'STUDENT' && (
                          <div className="flex-col gap-2 animate-fade-in">
                             <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--accent-primary)' }}>Assign Faculty Guide</label>
                             <div style={{ position: 'relative' }}>
                                <BadgeCheck size={16} color="var(--accent-primary)" style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', zIndex: 1 }} />
                                <select 
                                  className="input-premium" 
                                  style={{ paddingLeft: '2.5rem', borderColor: 'var(--accent-primary)', appearance: 'none', cursor: 'pointer', backgroundImage: 'url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%239ca3af%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E")', backgroundRepeat: 'no-repeat', backgroundPosition: 'right 1rem top 50%', backgroundSize: '0.65rem auto' }}
                                  ...
                                  required
                                >
                                  <option value="" disabled style={{ color: 'black' }}>Select a Faculty Member</option>
                                  {users?.filter((u: any) => u.role === 'GUIDE').map((guide: any) => (
                                    <option key={guide.id} value={guide.registrationNumber || guide.id.toString()} style={{ color: 'black' }}>
                                      {guide.name} ({guide.registrationNumber || `ID: ${guide.id}`})
                                    </option>
                                  ))}
                                </select>
                             </div>
